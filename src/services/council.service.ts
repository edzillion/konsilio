 ...
 * Council Service
 * 
 * Orchestrates the council of experts analysis with dependency injection.
 * Features:
...
/**
 * CouncilService - Orchestrates expert analysis
 */
export class CouncilService {
  ...
  async run(params: CouncilParams, options?: CouncilOptions, correlationId?: string): Promise<CouncilResult> {
    ...
    if (options?.debateMode) {
      ...
      const debatePromises = personas
        ...
        .map((p) => this.callExpert(p, expertModel, this.buildDebateMessage(p, userMessage, successfulReports), this.deps.config.timeouts.debateMs, log));
    ...
    const leadMessages: Message[] = [
      ...
      { role: 'user', content: this.buildLeadMessage(params, allReportsForLead) },
    ];
    const finalBlueprint = await this.deps.openRouterService.call({
    ...
    try { this.deps.databaseService.saveCouncilResult(result, params.draftPlan, params.techStack, params.contextConstraints, correlationId); } catch {}
  ...
  private async callExpert(persona: Persona, model: string, userMessage: string, timeoutMs: number, log: Logger | CorrelatedLogger): Promise<ExpertReport> {
    ...
    const content = await this.deps.openRouterService.call({ model, messages, maxTokens: 4096, temperature: 0.3, timeoutMs });
  ...
  private buildLeadMessage(params: CouncilParams, reports: ExpertReport[]): string {
  ...
  private buildDebateMessage(persona: Persona, originalMessage: string, allReports: ExpertReport[]): string {
